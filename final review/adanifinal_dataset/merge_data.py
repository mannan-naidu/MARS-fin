...
print("Starting the dataset merging process (v6 reading announcement dates from Excel)...")

# --- Configuration ---
OUTPUT_FILE = 'model_ready_merged_dataset_announced.csv'
START_DATE_FINAL = '2015-01-01' # User specified start date

# --- FILENAMES TO CHECK ---
BASE_FILE = 'ADANIENT.csv'
SENTIMENT_FILE = 'daily_news_features_ADANIENT.csv'
MACRO_FILE = 'macro_data_fred_detailed filled.csv'
QUARTERLY_FILE = 'adani_quarterly_reports.xlsx' 
# --- FIX: Use the Excel filename for announcement dates ---
ANNOUNCEMENT_DATE_FILE = 'adani quarterly release dates.xlsx' 
...
try:
    # --- 1. Load Announcement Dates from EXCEL ---
    print(f"Loading Quarterly Announcement Dates ({ANNOUNCEMENT_DATE_FILE})...")
    # --- FIX: Use pd.read_excel for the .xlsx file ---
    try:
        # Assuming data is on Sheet3 based on previous upload info
        df_announce_dates = pd.read_excel(ANNOUNCEMENT_DATE_FILE, sheet_name='Sheet3')
    except Exception as e:
        print(f"Error reading Excel file '{ANNOUNCEMENT_DATE_FILE}' on Sheet3: {e}")
        print("Trying Sheet1...")
        try:
             df_announce_dates = pd.read_excel(ANNOUNCEMENT_DATE_FILE, sheet_name='Sheet1')
        except Exception as e2:
             print(f"--- ERROR reading announcement dates Excel file. Tried Sheet3 & Sheet1. Error: {e2}")
             sys.exit()
    ...
    if 'Fiscal Year' not in df_announce_dates.columns:
        ...
        print(f"Found columns: {df_announce_dates.columns.tolist()}")
        print("Please ensure the Excel file has the correct headers.")
        sys.exit()

    # Melt the table to long format
    quarter_cols = [col for col in df_announce_dates.columns if col not in ['Fiscal Year']]
    if not quarter_cols:
         print(f"--- ERROR --- Could not find quarter columns (e.g., Q1, Q2) in {ANNOUNCEMENT_DATE_FILE}.")
         sys.exit()

    df_announce_dates = df_announce_dates.melt(id_vars=['Fiscal Year'],
                                             value_vars=quarter_cols,
                                             var_name='Quarter',
                                             value_name='Announcement Date')

    # --- Map Quarter String to Quarter End Date ---
    def map_quarter_to_date(row):
        ...
        try:
            ...
            start_year = int(year_part.split('-')[0])
             ...
             try: start_year = int(fiscal_year_str)
                 ...
                 print(f"Warning: Could not parse fiscal year '{fiscal_year_str}'. Skipping row.")
                 return None
        ...
        if 'Q1' in quarter_str or 'Apr-Jun' in quarter_str: return pd.Timestamp(year=start_year, month=6, day=30)
            ...
            print(f"Warning: Could not parse quarter '{quarter_str}'. Skipping row.")
            return None

    df_announce_dates['Quarter End Date'] = df_announce_dates.apply(map_quarter_to_date, axis=1)

    # Convert Announcement Date (might already be datetime if read from Excel)
    ...
    # --- 2. Load and Transform Quarterly Data ---
    print(f"Loading and Transforming Quarterly Data ({QUARTERLY_FILE})...")
    try:
        df_q = pd.read_excel(QUARTERLY_FILE, sheet_name='Sheet2', index_col=0, header=0)
    except Exception as e:
        print(f"Warning: Could not read 'Sheet2'. Trying 'Sheet1'. Error: {e}")
        try: df_q = pd.read_excel(QUARTERLY_FILE, sheet_name='Sheet1', index_col=0, header=0)
        except Exception as e2:
             print(f"--- ERROR reading quarterly data Excel file '{QUARTERLY_FILE}'. Error: {e2}")
             sys.exit()

    df_q = df_q.T
    try: df_q.index = pd.to_datetime(df_q.index, format="%b '%y") + pd.offsets.MonthEnd(0)
    except ValueError as e:
        print(f"--- ERROR parsing quarterly report dates: {e}. Ensure headers match '%b '%y'.")
        sys.exit()
    ...
    if 'Announcement Date' not in df_q_merged.columns or df_q_merged['Announcement Date'].isnull().all():
        ...
        print("Quarterly Index:", df_q.index.unique())
        print("Announce Map Index:", df_announce_map.index.unique())
        sys.exit()
    ...
    try:
        ...
        if not announce_dates_in_range.empty:
            ...
            q_col_name_base = [col for col in df_q_final.columns if 'Sales' in col][0]
    ...
    print("\n--- ERROR --- Missing required library. Please run: pip install openpyxl")
