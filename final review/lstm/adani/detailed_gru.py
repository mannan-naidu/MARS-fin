...
print("Starting LSTM/GRU *Single-Output* (Delta) Model - DOMESTIC EVENTS + TA FEATURES...")
...
# --- Output filenames ---
RESULTS_FILE = f'{STOCK_TICKER}_single_output_delta_{MODEL_ARCH_NAME}_domestic_events_plus_ta.json'
MODEL_SUMMARY_FILE = f'{STOCK_TICKER}_single_output_delta_{MODEL_ARCH_NAME}_domestic_events_plus_ta_model_summary.txt'
PREDICTION_PLOT_FILE = f'{STOCK_TICKER}_single_output_delta_{MODEL_ARCH_NAME}_domestic_events_plus_ta_predictions.png'
...
# ### <<< MODIFICATION: Robust TA calculation >>> ###
...
try:
    ...
    # 3. Bollinger Bands (Volatility) - More robust method
    bbands = ta.bbands(df['Close'], length=20, std=2)
    # Add the 3 bands directly
    ...
    # If it fails, TA_FEATURES will be an empty list and the script will continue

# --- 4. Feature Selection (Domestic Events + TA) ---
# ### <<< MODIFICATION: New Feature Hypothesis >>> ###
print("Defining 'Domestic Events + TA' features...")
FEATURES = [
    'Open', 'High', 'Low', 'Close', 'Volume', 
    'sentiment_weighted_mean', 'news_count',
]
FEATURES.extend(TA_FEATURES) # Add the TA features we just made

# New Hypothesis: Use Domestic macro features
BASE_DOMESTIC_FEATURES = [
    'CPI_Index',
    'IIP_Index',
    'GDP_Growth_QoQ',
    'Policy_Repo_Rate', # Keep this
    'Net_Profit_Loss_For_the_Period' # Keep this
]
available_cols = df.columns.tolist()

for base_name in BASE_DOMESTIC_FEATURES:
...
print(f"Using {len(FEATURES)} 'Domestic Events + TA' features: {FEATURES}")
...
results = {
    'model_name': f"{STOCK_TICKER}_SingleOutput_Delta_DomesticEvents_TA",
...
plt.title(f'{STOCK_TICKER} Price CHANGE Prediction ({MODEL_ARCH_NAME} - Domestic Events + TA)')
plt.xlabel('Date')
