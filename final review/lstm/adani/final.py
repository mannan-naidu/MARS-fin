...
# ### <<< MODIFICATION >>> ###
print("Starting LSTM/GRU *Single-Output* (Delta) Model - PURE TECHNICAL & SENTIMENT...")
...
# --- Output filenames ---
RESULTS_FILE = f'{STOCK_TICKER}_single_output_delta_{MODEL_ARCH_NAME}_pure_tech_sentiment.json'
MODEL_SUMMARY_FILE = f'{STOCK_TICKER}_single_output_delta_{MODEL_ARCH_NAME}_pure_tech_sentiment_model_summary.txt'
PREDICTION_PLOT_FILE = f'{STOCK_TICKER}_single_output_delta_{MODEL_ARCH_NAME}_pure_tech_sentiment_predictions.png'
...
# ### <<< MODIFICATION: More robust TA calculation >>> ###
...
# --- 4. Feature Selection (Pure Tech & Sentiment) ---
# ### <<< MODIFICATION: New Feature Hypothesis >>> ###
print("Defining 'Pure Technical & Sentiment' features...")
FEATURES = [
...
for feature in BASE_SENTIMENT_FEATURES:
    if feature in available_cols:
        ...
        print(f"--- WARNING --- Skipping sentiment feature '{feature}' (not found).")

# --- We are now intentionally *ignoring* all macro/fundamental features ---

FEATURES = sorted(list(set(FEATURES)))
print(f"Using {len(FEATURES)} 'Pure Tech & Sentiment' features: {FEATURES}")
...
results = {
    'model_name': f"{STOCK_TICKER}_SingleOutput_Delta_PureTechSentiment",
...
plt.title(f'{STOCK_TICKER} Price CHANGE Prediction ({MODEL_ARCH_NAME} - Pure Tech & Sentiment)')
plt.xlabel('Date')
