...
# ### <<< MODIFICATION >>> ###
print("Starting LSTM/GRU Multi-Output (Price CHANGE/DELTA) Model - EVENTS ONLY FEATURES...")
...
# You can keep tuning these variables
TUNING_VARS = {
    "SEQUENCE_LENGTH": 30,
    "LSTM_UNITS": 128,
    "DROPOUT": 0.2,
...
# --- Output filenames ---
# ### <<< MODIFICATION >>> ###
# Filenames now reflect the "events_only" feature set
RESULTS_FILE = f'lstm_multi_output_delta_{MODEL_ARCH_NAME}_events_only.json'
MODEL_SUMMARY_FILE = f'lstm_multi_output_delta_{MODEL_ARCH_NAME}_events_only_model_summary.txt'
PREDICTION_PLOT_FILE = f'lstm_multi_output_delta_{MODEL_ARCH_NAME}_events_only_predictions.png'
CM_PLOT_FILE = f'lstm_multi_output_delta_{MODEL_ARCH_NAME}_events_only_confusion_matrix.png'
...
# Start with Price/Volume and base Sentiment
FEATURES = [
    'Open', 'High', 'Low', 'Close', 'Volume', 
    'sentiment_weighted_mean', 'news_count',
]

# Base names of SHAP-selected fundamental features
BASE_SHAP_FEATURES = [
    'Crude_Oil_Brent', 
...
# Dynamically build the feature list
for base_name in BASE_SHAP_FEATURES:
    # We NO LONGER add the base_name (value) column
    chg_col = f"{base_name}_abs_change"
    stale_col = f"{base_name}_days_since_change"

    # Check if *only* the 2 event components exist
...
print(f"Using {len(FEATURES)} 'Events Only' features: {FEATURES}") # Should be 17 features
...
results = {
    'model_name': f"{FILENAME.split('_[0]')}_LSTM_MultiOutput_Delta_EventsOnly",
...
plt.title(f'{FILENAME.split("_")[0]} Price CHANGE Prediction ({MODEL_ARCH_NAME} - Events Only)')
plt.xlabel('Date')
...
plt.title('Confusion Matrix - Price Direction Prediction Head (Events Only)')
...
print("\n--- LSTM/GRU Multi-Output Delta Model Script Finished ---")