...
# ### <<< MODIFICATION >>> ###
print("Starting Final GRU Single-Output (Delta) Model - SENTIMENT & NEWS CATEGORIES (No TA)...")
...
# --- Output filenames ---
# ### <<< MODIFICATION >>> ###
RESULTS_FILE = f'{STOCK_TICKER}_FINAL_model_Sentiment_Categories_results.json'
MODEL_SUMMARY_FILE = f'{STOCK_TICKER}_FINAL_model_Sentiment_Categories_summary.txt'
PREDICTION_PLOT_FILE = f'{STOCK_TICKER}_FINAL_model_Sentiment_Categories_predictions.png'
CM_PLOT_FILE = f'{STOCK_TICKER}_FINAL_model_Sentiment_Categories_derived_cm.png'
...
# --- 3. Feature Selection (Sentiment + Categories) ---
# ### <<< MODIFICATION: Removed TA Features >>> ###
print("Defining 'Sentiment & News Categories' features...")
FEATURES = [
...
# .dropna() is critical - removes NaNs from .diff()
...
# --- 13. Calculate DERIVED Classification Metrics ---
print("\n--- DERIVED Classification Metrics (from Regression) ---")
...
print(f"DERIVED Directional Accuracy: {accuracy:.4f}")
print(f"DERIVED Directional Precision (for 'Up'): {precision:.4f}")
print(f"DERIVED Directional Recall (for 'Up'): {recall:.4f}")
print(f"DERIVED Directional F1-Score (for 'Up'): {f1:.4f}")
print("DERIVED Confusion Matrix:")
print(cm)
print("\nDERIVED Classification Report:")
print(report)

# --- 14. Save Metrics and Features to File ---
print(f"\nSaving results to {RESULTS_FILE}...")
results = {
    'model_name': f"{STOCK_TICKER}_SingleOutput_Delta_SentimentCategories",
    ...
    'derived_classification_metrics': {
        'accuracy': accuracy,
...
plt.title(f'{STOCK_TICKER} Price CHANGE Prediction ({MODEL_ARCH_NAME} - Sentiment & News Categories)')
plt.xlabel('Date')
...
print(f"Plotting derived confusion matrix...")
...
plt.title('DERIVED Confusion Matrix (Sentiment & News Categories)')
...
print(f"\n--- {MODEL_ARCH_NAME} Single-Output Delta Model Script Finished ---")