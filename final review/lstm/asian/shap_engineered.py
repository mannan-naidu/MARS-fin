...
# ### <<< MODIFICATION >>> ###
print("Starting LSTM Single-Output SHAP-Engineered-Features Model Script...")
...
# --- Output filenames ---
# ### <<< MODIFICATION >>> ###
RESULTS_FILE = 'lstm_single_output_shap_engineered_results.json'
MODEL_SUMMARY_FILE = 'lstm_single_output_shap_engineered_model_summary.txt'
PREDICTION_PLOT_FILE = 'lstm_single_output_shap_engineered_predictions.png'
...
FEATURES = [
    # Price/Volume are ESSENTIAL
    'Open', 'High', 'Low', 'Close', 'Volume', 
    
    # Top Sentiment (no engineered factors needed)
...
# Dynamically build the full feature list with engineered components
...
print(f"Using {len(FEATURES)} features: {FEATURES}") # Should be 22 features
...
results = {
    'model_name': f"{FILENAME.split('_')[0]}_LSTM_SingleOutput_SHAP_Engineered",
...
plt.title(f'{FILENAME.split("_")[0]} Close Price Prediction (LSTM SHAP+Engineered Features)')
plt.xlabel('Date')
